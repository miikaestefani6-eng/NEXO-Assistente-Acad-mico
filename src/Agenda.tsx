...
function Agenda() {
  ...
  const initialItems = (() => { const content = cmsItems(); const committedMinutes = content.filter((item) => item.source === "cms").reduce((sum, item) => sum + item.duration, 0); const planningMinutes = Math.max(0, availableMinutes - committedMinutes); const planned = planningMinutes >= 15 ? planItems(generateAdaptivePlan(getWorkload(), planningMinutes, 7)[0].missions) : []; const base = [...content, ...planned].map((item, index) => ({ ...item, status: index === 0 ? "next" as const : item.status === "next" ? "pending" as const : item.status })); const reviews: AgendaItem[] = dueLearningGaps().slice(0, 2).map((gap, index) => ({ id: `learning-${gap.id}`, gapId: gap.id, time: index === 0 ? "Revisão" : "Depois", subject: gap.discipline, title: `Revisar: ${gap.topic}`, type: "Revisão adaptativa", duration: 15, status: "pending", source: "learning" })); return reviews.length ? [...reviews, ...base].map((item, index) => ({ ...item, status: index === 0 ? "next" as const : item.status === "next" ? "pending" as const : item.status })) : base; })();
