...
import { activeLearningGaps, dueLearningGaps, recordLearningGap } from "./lib/nexo/learningMemory";

type AgendaItem = { id: string; cmsId?: string; time: string; subject: string; title: string; type: string; duration: number; status: "next" | "pending" | "done"; source: "cms" | "event" | "plan" };
...
function Agenda() {
  ...
  const initialItems = (() => { const content = cmsItems(); return content.length ? content : planItems(generateAdaptivePlan(getWorkload(), 90, 7)[0].missions); })();
