...
import { dueLearningGaps, reinforceLearningGap } from "./lib/nexo/learningMemory";
...
function planItems(missions: ReturnType<typeof generateAdaptivePlan>[number]["missions"]): AgendaItem[] { return missions.map((mission, index) => ({ id: mission.id, time: index === 0 ? "Agora" : "A seguir", subject: mission.subject, title: mission.title, type: mission.type, duration: mission.duration, status: index === 0 ? "next" : "pending", source: "plan" })); }
...
function cmsItems(): AgendaItem[] {
  ...
  // Compromissos com horário real ficam em ordem cronológica. Atividades flexíveis
  // vêm depois, sem comparar "Agora/A seguir" com strings de relógio.
  const all = [...eventItems, ...activityItems];
...
function Agenda() {
  ...
  const initialItems = (() => { const content = cmsItems(); const reviews: AgendaItem[] = dueLearningGaps().slice(0, 2).map((gap, index) => ({ id: `learning-${gap.id}`, gapId: gap.id, time: index === 0 ? "Revisão" : "Depois", subject: gap.discipline, title: `Revisar: ${gap.topic}`, type: "Revisão adaptativa", duration: 15, status: "pending", source: "learning" })); const committedMinutes = content.filter((item) => item.source === "cms").reduce((sum, item) => sum + item.duration, 0) + reviews.reduce((sum, item) => sum + item.duration, 0); const planningMinutes = Math.max(0, availableMinutes - committedMinutes); const planned = planningMinutes >= 15 ? planItems(generateAdaptivePlan(getWorkload(), planningMinutes, 7)[0].missions) : []; const base = [...reviews, ...content, ...planned]; return base.map((item, index) => ({ ...item, status: index === 0 ? "next" as const : item.status === "next" ? "pending" as const : item.status })); })();
